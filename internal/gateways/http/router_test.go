...
func TestSensorsHistoryRoutes(t *testing.T) {
	t.Run("GET_sensors_sensor_id_history", func(t *testing.T) {
		...
		t.Run("params_has_invalid_format_400", func(t *testing.T) {
			w := httptest.NewRecorder()
			...
			req, _ = http.NewRequest(http.MethodGet, "/sensors/1/history?start_date=123", nil)
			req.Header.Add("Accept", "application/json")
			router.ServeHTTP(w, req)

			assert.Equal(t, http.StatusBadRequest, w.Code, "Получили в ответ не тот код")

			req, _ = http.NewRequest(http.MethodGet, "/sensors/1/history?end_date=123", nil)
			req.Header.Add("Accept", "application/json")
			router.ServeHTTP(w, req)
		...
		t.Run("user_doesnt_exist_404", func(t *testing.T) {
			...
			req, _ := http.NewRequest(http.MethodGet, "/sensors/3/history", nil)
			req.Header.Add("Accept", "application/json")
			router.ServeHTTP(w, req)
